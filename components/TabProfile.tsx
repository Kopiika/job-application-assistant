    ...
    <div style={{ borderBottom: '1px solid var(--border)', paddingBottom: 14, marginBottom: 6 }}>
      <div style={{ display: 'flex', gap: 16, alignItems: 'center', paddingTop: 14 }}>
        ...
        <div>
          ...
          <div
            style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 8, lineHeight: 1.4 }}
          >
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            {photo && (
              <button
                ...
                style={{ ...smallBtn, color: 'var(--danger, #dc2626)' }}
              >
                Remove
              </button>
            )}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingTop: 12 }}>
      ...
      <STA
        ...
        placeholder="Brief context about this role..."
        rows={2}
      />
      <STA
        ...
        value={entry.bullets}
        onChange={s('bullets')}
        placeholder={'Built a 5-step registration form...\nDeveloped full admin panel...'}
        rows={4}
      />
      <SF
        label="Tech stack"
        value={entry.tech}
        onChange={s('tech')}
        placeholder="React 19 · TypeScript · Tailwind CSS · Git"
      />
...
function ProjEditor({
  ...
  const s =
    (key: keyof ProjectEntry) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
      onChange({ ...entry, [key]: e.target.value })
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingTop: 12 }}>
      <FormRow>
        <SF
          ...
          value={entry.name}
          onChange={s('name')}
          placeholder="LARP Event Registration Platform"
          flex={2}
        />
      ...
      <FormRow>
        <SF
          ...
          onChange={s('githubLink')}
          placeholder="github.com/user/repo"
          flex={1}
        />
        <SF
          label="Live demo link"
          value={entry.demoLink}
          onChange={s('demoLink')}
      ...
      <STA
        ...
        value={entry.bullets}
        onChange={s('bullets')}
        placeholder={'Token-based invite system...\n100% Lighthouse score...'}
        rows={3}
      />
      <SF
        label="Tech stack"
        value={entry.tech}
        onChange={s('tech')}
        placeholder="React 19 · TypeScript · Vite · Tailwind CSS 4"
      />
      <EntryActions onDone={onDone} onRemove={onRemove} />
    </div>
  )
...
function EduEditor({
  ...
  const s =
    (key: keyof EducationEntry) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
      onChange({ ...entry, [key]: e.target.value })
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingTop: 12 }}>
      <SF
        ...
        value={entry.degree}
        onChange={s('degree')}
        placeholder="Vocational Qualification – Software Development"
      />
      <FormRow>
        <SF
          ...
          value={entry.school}
          onChange={s('school')}
          placeholder="Business College Helsinki"
          flex={2}
        />
        <SF
          label="From"
      ...
      <STA
        ...
        value={entry.description}
        onChange={s('description')}
        placeholder="Courses, specializations, notable projects..."
        rows={2}
      />
      <EntryActions onDone={onDone} onRemove={onRemove} />
    </div>
...
function ProjList({
  ...
  return (
    <EntrySection
      label="Projects"
      hint="Most relevant projects are reordered by the AI per job."
      onAdd={add}
    ...
    >
      {entries.map((e, i) => (
        <EntryCard
          ...
          sub={[e.githubLink, e.demoLink].filter(Boolean).join('  ·  ')}
          date={[e.from, e.to].filter(Boolean).join(' – ')}
          isExpanded={expanded === i}
          onToggle={() => setExpanded(expanded === i ? null : i)}
          onRemove={() => rem(i)}
      ...
      {entries.map((e, i) => (
        <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
          <SF
            label="Language"
            value={e.language}
            onChange={(ev) => upd(i, { ...e, language: ev.target.value })}
            placeholder="English"
            flex={1}
          />
          <SF
            label="Level"
            value={e.level}
            onChange={(ev) => upd(i, { ...e, level: ev.target.value })}
            placeholder="Working proficiency"
            flex={2}
          />
          <button
            type="button"
            onClick={() => rem(i)}
            style={{ ...iconBtn, color: 'var(--danger, #dc2626)', marginBottom: 1, height: 32 }}
          >
            <IconTrash size={14} />
          </button>
        </div>
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 10 }}>
            <span
              className="font-serif"
              style={{ fontSize: 32, fontWeight: 500, lineHeight: 1, color: 'var(--text)' }}
            >
              {completion.score}
            </span>
        ...
        >
          ...
          <div style={{ fontSize: 13, color: 'var(--text-muted)', lineHeight: 1.55 }}>
            Write bullets in outcome-focused style: "Built X with Y, reduced Z by N%". Each bullet
            becomes a bullet point in the downloaded DOCX.
          </div>
        </div>
      </div>
    </div>
    ...
    <div>
      ...
      >
        <div>
          ...
          {hint && (
            <div style={{ fontSize: 12, color: 'var(--text-subtle)', marginTop: 2 }}>{hint}</div>
          )}
        </div>
        <button
          type="button"
...
function Field({
  ...
  return (
    <label style={{ display: 'block' }}>
      ...
      />
      {hint && (
        <div style={{ fontSize: 12, color: 'var(--text-subtle)', marginTop: 4 }}>{hint}</div>
      )}
    </label>
  )
}
