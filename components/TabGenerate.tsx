...
const STEPS = ['Reading the job description', 'Matching your skills', 'Drafting CV', 'Writing cover letter']
...
export default function TabGenerate({ profile, onSaveApplication }: TabGenerateProps) {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setGenState({ status: 'error', message: err instanceof Error ? err.message : 'Something went wrong' })
    }
  }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', boxShadow: 'var(--shadow-sm)', padding: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-subtle)' }}>
                Job description
              </span>
              <span style={{ fontSize: 12, color: 'var(--text-subtle)', fontFamily: 'var(--font-mono), monospace' }}>
                {wordCount} words
              </span>
            </div>
            <textarea
              ...
              style={{
                ...
                background: 'var(--surface)', fontSize: 13.5, lineHeight: 1.6,
            ...
            <div style={{ marginTop: 10, display: 'flex', gap: 8 }}>
              <button
                onClick={() => setJd('')}
                style={{ fontSize: 12, color: 'var(--text-muted)', padding: '4px 10px', borderRadius: 999, background: 'var(--surface-2)', border: '1px solid var(--border)' }}
              >
                Clear
              </button>
            </div>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', boxShadow: 'var(--shadow-sm)', padding: 20 }}>
            <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-subtle)', display: 'block', marginBottom: 14 }}>
              What to generate
            </span>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <DocTypeRow icon={<IconFileText size={18} />} title="Full CV" desc="Complete CV rewritten and tailored to this role" checked={generateCV} onClick={() => setGenerateCV((v) => !v)} />
              <DocTypeRow icon={<IconMail size={18} />} title="Cover letter" desc="Personalized opening referencing the company" checked={generateCover} onClick={() => setGenerateCover((v) => !v)} />
            </div>

            {genState.status === 'error' && (
              <div style={{ marginTop: 14, padding: '10px 14px', background: 'var(--danger-soft)', borderRadius: 'var(--radius-sm)', fontSize: 13, color: 'var(--danger)' }}>
                {genState.message}
              </div>
            )}
            ...
            <button
              ...
              disabled={genState.status === 'generating' || !jd.trim() || (!generateCV && !generateCover)}
              style={{
                ...
                opacity: (genState.status === 'generating' || !jd.trim() || (!generateCV && !generateCover)) ? 0.6 : 1,
                cursor: (genState.status === 'generating' || !jd.trim() || (!generateCV && !generateCover)) ? 'not-allowed' : 'pointer',
                transition: 'background 120ms',
              }}
            >
              {genState.status === 'generating' ? (
                <><IconRefresh size={16} style={{ animation: 'spin 800ms linear infinite' }} /> Generating…</>
              ) : (
                <><IconSparkles size={16} /> {generateCV && generateCover ? 'Generate both documents' : generateCV ? 'Generate CV summary' : 'Generate cover letter'}</>
              )}
            </button>
          </div>
        </div>
...
function DocTypeRow({ icon, title, desc, checked, onClick }: { icon: React.ReactNode; title: string; desc: string; checked: boolean; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      style={{
...
function EmptyState() {
  return (
    <div style={{
      padding: '60px 40px', textAlign: 'center', minHeight: 520,
      ...
      <div className="font-serif" style={{ fontSize: 22, fontWeight: 500, marginBottom: 6, color: 'var(--text)' }}>
        Ready when you are
      </div>
...
function GeneratingState() {
  return (
    <div style={{
      padding: 32, minHeight: 520, display: 'flex', flexDirection: 'column', justifyContent: 'center',
...
function SaveModal({ onConfirm, onClose }: { onConfirm: (company: string, role: string) => void; onClose: () => void }) {
  ...
  return (
    <div
      onClick={onClose}
      style={{
        position: 'fixed', inset: 0, background: 'oklch(0.2 0.012 70 / 0.4)',
    ...
    >
      <div
        ...
        style={{
          ...
          animation: 'popIn 200ms ease', overflow: 'hidden',
        }}
      >
        <div style={{ padding: '18px 22px', borderBottom: '1px solid var(--border)', fontSize: 16, fontWeight: 600 }}>
          Save to tracker
        </div>
        <div style={{ padding: 22, display: 'flex', flexDirection: 'column', gap: 14 }}>
          <Field label="Company" value={company} onChange={setCompany} placeholder="e.g. Wolt" />
          <Field label="Role" value={role} onChange={setRole} placeholder="e.g. Junior Frontend Developer" />
        </div>
        <div style={{ padding: '14px 22px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'flex-end', gap: 8, background: 'var(--surface-2)' }}>
          <button onClick={onClose} style={{ padding: '8px 14px', borderRadius: 'var(--radius-sm)', fontSize: 14, color: 'var(--text)', border: '1px solid transparent' }}>
            Cancel
          </button>
          <button
            onClick={() => onConfirm(company || 'Unknown', role || 'Unknown')}
            style={{
              padding: '8px 14px', borderRadius: 'var(--radius-sm)', fontSize: 14, fontWeight: 500,
