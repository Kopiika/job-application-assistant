...
export default function DriveUpload({ cvSummary, coverLetter, candidateName }: DriveUploadProps) {
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
      ...
      {state.status === 'loading' && <p className="mt-3 text-xs text-zinc-400">{state.step}</p>}

      {state.status === 'error' && (
        <p className="mt-3 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600">{state.message}</p>
      )}
...
async function fetchDocxAsBase64(
  text: string,
  type: 'cv' | 'cover',
  name: string
): Promise<string> {
...
function DriveIcon({ size = 18 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 87.3 78" fill="none">
      <path
        d="M6.6 66.85l3.85 6.65c.8 1.4 1.95 2.5 3.3 3.3L29.9 48H0c0 1.55.4 3.1 1.2 4.5z"
        fill="#0066DA"
      />
      <path
        d="M43.65 25L27.5 0c-1.35.8-2.5 1.9-3.3 3.3L1.2 43.5C.4 44.9 0 46.45 0 48h29.9z"
        fill="#00AC47"
      />
      <path
        d="M73.55 76.8c1.35-.8 2.5-1.9 3.3-3.3l1.6-2.75 7.65-13.25c.8-1.4 1.2-2.95 1.2-4.5H57.4l6.2 11.9z"
        fill="#EA4335"
      />
      <path
        d="M43.65 25L57.4 48h29.9c0-1.55-.4-3.1-1.2-4.5L64.2 3.3c-.8-1.4-1.95-2.5-3.3-3.3z"
        fill="#00832D"
      />
      <path
        d="M57.4 48H29.9L13.75 76.8c1.35.8 2.9 1.2 4.5 1.2h50.8c1.6 0 3.15-.45 4.5-1.2z"
        fill="#2684FC"
      />
      <path
        d="M73.4 25.5L61.05 3.3c-.8-1.4-1.95-2.5-3.3-3.3L43.65 25 57.4 48l29.75-.05c0-1.55-.4-3.1-1.2-4.5z"
        fill="#FFBA00"
      />
    </svg>
  )
